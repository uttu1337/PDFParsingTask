...
public class PdfParser {
    ...
    private void putTaxpayerIdentificationNumber(Map<String, String> result, String line) {
        ...
        result.put(TAXPAYER_IDENTIFICATION_NUMBER_FIELD, line.replace(TAXPAYER_IDENTIFICATION_NUMBER_SUBSTRING+" ", "").trim());
    }

    private void putDocumentTypeSeriesAndNumber(Map<String, String> result, String line) {

        String[] partsOfDocument = line.replace(DOCUMENT_TYPE_SUBSTRING+":", "")
                .split(DOCUMENT_SERIES_AND_NUMBER_SUBSTRING+" ");

        if (partsOfDocument.length == 2) {

            result.put(DOCUMENT_TYPE_FIELD, partsOfDocument[0].trim());
            result.put(SERIES_AND_NUMBER_FIELD, partsOfDocument[1].replaceAll("\\s", "").trim());
        }
    }

    private void putESignatureValue(Map<String, String> result, String line) {

        result.put(E_SIGNATURE_VALUE_FIELD, line.replace(CERTIFICATE_SUBSTRING + ": ", "").trim());
    }

    private void putESignatureOwner(Map<String, String> result, String line) {

        result.put(E_SIGNATURE_OWNER_FIELD, line.replace(OWNER_SUBSTRING + ": ", "").trim());
    }

    private void putProductionAndExpirationDate(Map<String, String> result, String line) {
        String[] partsOfDates = line.replace(PRODUCTION_DATE_SUBSTRING, "")
